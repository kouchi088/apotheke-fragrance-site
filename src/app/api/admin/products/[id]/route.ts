...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data, error } = await db.from('products').select('*').eq('id', params.id).is('deleted_at', null).single();
  if (error || !data) return fail('NOT_FOUND', 'product not found', 404);
  return ok(data);
}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const db = getSupabaseAdminClient();
  const { data: before } = await db.from('products').select('*').eq('id', params.id).single();
  const { error } = await db.from('products').update({ deleted_at: new Date().toISOString(), is_published: false }).eq('id', params.id);
