...
const ORDER_SORT_PRIORITY: Record<string, number> = {
  completed: 0,
  processing: 1,
  cancelled: 2,
  shipped: 3,
...
function sortOrdersForAdmin(a: any, b: any) {
  const priorityA = ORDER_SORT_PRIORITY[a.status ?? ''] ?? 99;
  const priorityB = ORDER_SORT_PRIORITY[b.status ?? ''] ?? 99;
...
export default async function AdminOrdersPage({ searchParams }: AdminOrdersPageProps) {
  ...
  }).sort(sortOrdersForAdmin);

  const activeOrders = filteredOrders.filter((order: any) => order.status !== 'shipped');
  ...
  return (
    <div className="space-y-6">
      ...
      {shippedOrders.length > 0 && (
        <details className="rounded-xl border border-stone-200 bg-white" open={Boolean(statusFilter === 'shipped')}>
          <summary className="cursor-pointer list-none px-4 py-3 text-sm font-medium text-stone-700">
            発送済みの注文を表示 ({shippedOrders.length})
          </summary>
